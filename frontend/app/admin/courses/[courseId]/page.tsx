...
const Page = async ({params}: { params: Params }) => {
    ...
    const {data:course} = await adminGetCourseById(courseId);
    if (!course) notFound();
    ...
    return (
        <div>
            <h1 className={"text-xl font-bold mb-8"}>
                Preview Course : <span className={"text-primary underline"}>{course.title}</span>
            </h1>
                                    ...
                                    course.price === 0 ?
                                        <p className={"text-xs text-muted-foreground mt-4 text-center"}>
                                            <EnrollmentButton courseId={course.id} courseTitle={course.title}
                                                              btnLabel={"Enroll Free Course"} price={finalPrice}/>
                                        </p>
                                        : <>
                                            <EnrollmentButton courseId={course.id} courseTitle={course.title}
                                                              btnLabel={"Enroll Now !"} price={finalPrice}/>
                                            <p className={"text-xs text-muted-foreground mt-4 text-center"}>
